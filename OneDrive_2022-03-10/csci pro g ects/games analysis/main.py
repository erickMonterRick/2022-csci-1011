...
df_main = pd.read_csv("game-ratings-by-top-ten-genres.csv")

df_follow = df_main.groupby(["genre_name"])["follow count"].sum().reset_index()
...
df_hype = df_hype.reame(columns = { "hype_count": "total_hypes" })

plt.bar(df_follow.index - BAR_WIDTH / 2, df_follow["total_follows"], color = "blue", label = "Number of Follows", width = BAR_WIDTH)

plt.bar(df_hype.index + BAR_WIDTH / 2, df_hype["total_hypes"], color = "red", label = "Number of Hypyes", width = BAR_WIDTH)
...
df_main = pd.read_csv("game-ratings-by-top-ten-platforms.csv")

df_follow = df_main.groupby(["platform_name"])["follow count"].sum().reset_index()
